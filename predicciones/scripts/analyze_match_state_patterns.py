...
def load_all_summaries_from_cache(cache_dir: str = "data/cache/espn",
                                   ...
                                   max_matches: Optional[int] = None) -> List[Tuple[str, str, Dict[str, Any]]]:
    """
    Load all match summaries from cache directory with optional filters.
    ...
    Returns:
        List of (event_id, league, summary) tuples sorted by date
    """
    import os
    import re
            ...
            if match:
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        date_str = header.get('date') or header.get('startDate')
                        if date_str:
                            try:
                        ...
                        # Apply date filters
                        if from_date and match_date and match_date < from_date:
                            continue
                        if to_date and match_date and match_date > to_date:
                            continue
                        
                        results.append((event_id, league, summary, match_date))
                except Exception as e:
                    logger.debug(f"Error loading {filename}: {e}")
    
    # Sort by date (most recent first, or unknown dates last)
    results.sort(key=lambda x: x[3] if x[3] else datetime.max, reverse=True)
    
    # Apply max_matches limit
    if max_matches and len(results) > max_matches:
        results = results[:max_matches]
    
    # Return without the date field (maintain backward compatibility)
...
def run_match_state_analysis(
    ...
    Args:
        league_filter: Optional league to filter by
        days_back: Days back to consider (used if from_date not provided)
        from_date: Optional start date filter
        to_date: Optional end date filter
        max_matches: Optional maximum number of matches to process
    ...
    # Calculate from_date if not provided but days_back is
    if from_date is None and days_back:
    ...
    summaries = load_all_summaries_from_cache(
    ...
    all_windows = []
    all_transitions = []
    coverage_data = []  # For data coverage report
    ...
    for event_id, league, summary in summaries:
        ...
        if not context:
            coverage_data.append({
                ...
                'excluded_reason': 'Invalid context'
            })
            continue
        
        # Extract events
        events = extract_commentary_events(summary, event_id)
        key_events = extract_key_events(summary, event_id)
        ...
        if len(events) < 5:
            ...
            coverage_data.append({
                ...
                'cached_summary': True,
                'summary_valid': False,
                'commentary_count': len(events),
                'key_events_count': len(key_events),
                'windows_generated': 0,
                'transitions_generated': 0,
                'excluded_reason': f'Too few events ({len(events)})'
            })
            continue
        ...
        coverage_data.append({
            ...
            'commentary_count': len(events),
            'key_events_count': len(key_events),
            'windows_generated': len(fixed_windows) + len(anchored_windows),
            'transitions_generated': len(fixed_windows) + len(anchored_windows),  # Approximate
            'excluded_reason': ''
        })
    ...
    if coverage_data:
        with open(coverage_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                ...
                'cached_summary', 'summary_valid', 'commentary_count',
                'key_events_count', 'windows_generated', 'transitions_generated',
                'excluded_reason'
            ])
            writer.writeheader()
    ...
    return {
        ...
        'matches_valid': len([d for d in coverage_data if d['summary_valid']]),
...
Examples:
  ...
  python scripts/analyze_match_state_patterns.py --league fifa.world --use-all-cache --output-dir output/analysis_full
  python scripts/analyze_match_state_patterns.py --league fifa.world --from-date 2025-01-01 --to-date 2026-07-07 --use-all-cache
  ...
  python scripts/analyze_match_state_patterns.py --output-dir output/analysis
    ...
    parser.add_argument('--use-all-cache', action='store_true',
                        help='Use all cached summaries without date filtering')
    parser.add_argument('--fetch-missing', action='store_true',
                        help='Fetch missing matches from ESPN API (not yet implemented)')
    parser.add_argument('--min-pattern-sample', type=int, default=20,
                        help='Minimum sample size for robust pattern detection (default: 20)')
    ...
    if args.from_date:
        try:
            ...
            print(f"Error: Invalid from-date format '{args.from_date}'. Use YYYY-MM-DD.")
            exit(1)
    ...
    if args.to_date:
        try:
            ...
            print(f"Error: Invalid to-date format '{args.to_date}'. Use YYYY-MM-DD.")
            exit(1)
    
    # If --use-all-cache is set, disable date filtering
    days_back = args.days_back
    if args.use_all_cache:
        days_back = None  # Disable date filtering
        from_date = None
        to_date = None
    ...
    if 'error' in result:
        ...
        print(f"Matches valid: {result.get('matches_valid', 'N/A')}")
